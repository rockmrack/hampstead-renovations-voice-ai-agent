...
class MondayClient:
    ...
    async def create_item(
        self, board_id: str, item_name: str, column_values: dict | None = None
    ) -> dict:
    ...
    async def update_item(self, board_id: str, item_id: str, column_values: dict) -> dict:
...
class ProjectManagementService:
    ...
    def __init__(self):
        self.monday_client: MondayClient | None = None
        self.asana_client: AsanaClient | None = None
        self._active_platform: PMPlatform | None = None
        self._config: dict = {}
    ...
    async def initialize(
        ...
        if platform == PMPlatform.MONDAY:
            ...
            self._config = {"workspace_id": workspace_id, "project_id": project_id}
    ...
    async def create_project_from_lead(
        ...
        if self._active_platform == PMPlatform.MONDAY and self.monday_client:
            return await self._create_monday_item(
                client_name, client_phone, client_email, service_type, notes, source
            )
    ...
    async def _create_monday_item(
        ...
        try:
            ...
            result = await self.monday_client.create_item(
                self._config["board_id"], client_name, column_values
            )
            ...
            if notes and item_id:
                await self.monday_client.add_update(
                    item_id, f"📞 Voice AI Conversation Notes:\n\n{notes}"
                )
...
{notes or "No notes"}"""
    ...
    async def update_project_status(
        self, project_id: str, new_status: ProjectStatus, notes: str | None = None
    ) -> bool:
        """
        ...
        if self._active_platform == PMPlatform.MONDAY and self.monday_client:
            try:
                column_values = {
                    "status": self.MONDAY_STATUS_MAP.get(new_status, {"label": "Lead"})
                }

                await self.monday_client.update_item(
                    self._config["board_id"], project_id, column_values
                )

                if notes:
                    await self.monday_client.add_update(
                        project_id, f"Status updated to {new_status.value}:\n{notes}"
                    )
            ...
            try:
                ...
                if notes:
                    await self.asana_client.add_comment(
                        project_id, f"Status update: {new_status.value}\n\n{notes}"
                    )
    ...
    async def add_conversation_note(
        self, project_id: str, note: str, author: str = "Voice AI"
    ) -> bool:
        """
    ...
    async def get_project_summary(self, project_id: str) -> dict | None:
    ...
    async def schedule_followup(
        self, project_id: str, followup_date: datetime, reason: str
    ) -> bool:
        """Schedule a follow-up task"""
        return (
            await self.create_task_from_conversation(
                project_id,
                f"Follow-up: {reason}",
