...
class EmotionAnalysisService:
    ...
    EMOTION_INDICATORS = {
        EmotionType.HAPPY: [
            "wonderful", "fantastic", "great", "love", "perfect", "excellent",
            "amazing", "brilliant", "thank you so much", "really pleased"
        ],
        EmotionType.SATISFIED: [
            "good", "nice", "happy with", "that works", "sounds good", "fine",
            "okay great", "that's helpful", "appreciate"
        ],
        EmotionType.CONFUSED: [
            "don't understand", "what do you mean", "confused", "not sure",
            "can you explain", "lost me", "unclear", "what?", "sorry?",
            "could you repeat", "i'm not following"
        ],
        EmotionType.FRUSTRATED: [
            "frustrat", "annoying", "already told", "again", "how many times",
            "not working", "still waiting", "this is ridiculous", "unacceptable",
            "waste of time", "getting nowhere"
        ],
        EmotionType.ANGRY: [
            "furious", "outrageous", "disgusted", "terrible", "worst",
            "never again", "complaint", "legal", "trading standards", "ombudsman"
        ],
        EmotionType.ANXIOUS: [
            "worried", "nervous", "concerned", "scary", "anxious", "uncertain",
            "not sure if", "what if", "risky", "afraid"
        ],
        EmotionType.URGENT: [
        ...
        EmotionType.INTERESTED: [
            "tell me more", "interested", "curious", "how does that work",
            "what about", "can you", "i'd like to know", "sounds interesting"
        ],
        EmotionType.SKEPTICAL: [
            "really?", "are you sure", "seems expensive", "don't believe",
            "doubt", "sounds too good", "what's the catch", "hidden", "but"
        ]
    }
    ...
    async def analyze_text_emotion(
        ...
        if detected_emotions:
            ...
            if len(detected_emotions) > 1:
                emotions_sorted = sorted(detected_emotions.items(), key=lambda x: x[1], reverse=True)
        ...
        if primary in self.ESCALATION_TRIGGERS:
            ...
            if self._intensity_exceeds_threshold(intensity, threshold):
                escalate = True
                escalation_reason = f"{primary.value} emotion detected at {intensity.value} intensity"
                
        # Get suggested action
        adaptation = self.RESPONSE_ADAPTATIONS.get(primary, self.RESPONSE_ADAPTATIONS[EmotionType.NEUTRAL])
    ...
    async def _ai_emotion_analysis(
        self,
        text: str,
...
{{
    ...
    "indicators": ["list of emotional cues detected"]
            ...
            primary = EmotionType(primary_str) if primary_str in [e.value for e in EmotionType] else EmotionType.NEUTRAL
            
            secondary = None
            if result.get("secondary_emotion"):
                sec_str = result["secondary_emotion"]
                secondary = EmotionType(sec_str) if sec_str in [e.value for e in EmotionType] else None
                
            intensity_str = result.get("intensity", "low")
            intensity = EmotionIntensity(intensity_str) if intensity_str in [i.value for i in EmotionIntensity] else EmotionIntensity.LOW
    ...
    def _intensity_exceeds_threshold(
        self,
        intensity: EmotionIntensity,
        threshold: EmotionIntensity
    ) -> bool:
        """Check if intensity exceeds threshold"""
        levels = [EmotionIntensity.LOW, EmotionIntensity.MEDIUM, EmotionIntensity.HIGH, EmotionIntensity.CRITICAL]
    ...
    def _calculate_overall_sentiment(self, track: ConversationEmotionTrack) -> str:
        ...
        positive_count = sum(1 for _, e in recent_emotions if e.primary_emotion in positive_emotions)
        negative_count = sum(1 for _, e in recent_emotions if e.primary_emotion in negative_emotions)
    ...
    def get_response_adaptation(self, emotion: EmotionType) -> dict:
        ...
        return self.RESPONSE_ADAPTATIONS.get(emotion, self.RESPONSE_ADAPTATIONS[EmotionType.NEUTRAL])
    ...
    async def should_escalate_to_human(
        ...
        recent = track.emotion_timeline[-3:] if len(track.emotion_timeline) >= 3 else track.emotion_timeline
        negative_emotions = {EmotionType.FRUSTRATED, EmotionType.ANGRY, EmotionType.ANXIOUS}
